...
tokenizer_data = torch.load("subword-tokenizer.pth", map_location="cpu")
...
# prompts the user and outputs the model result
while True:
    prompt = input("Prompt:\n")
    context = torch.tensor(encode(prompt), dtype=torch.long, device=device)
    generated_tokens = decode(
        cuda_model.generate(context.unsqueeze(0), max_new_tokens=150)[0].tolist()
    )
    print(f"Completion:\n{generated_tokens}")
